...
# Conductor — Plans a track, generates track-specific spec documents and updates the tracks file
...
2.  **Get Track Description & Enter Plan Mode:**
    *   **If text the user provided after the slash command (if none, ask) is empty:**
        1. Ask the user to enable Plan mode in Cursor (or SwitchMode → plan).
        2. Ask the user using the `AskQuestion` tool (do not repeat the question in the chat):
            - **questions:**
                - **header:** "Description"
                - **type:** "text"
    ...
    *   **If text the user provided after the slash command (if none, ask) contains a description:**
        1. Use the content of text the user provided after the slash command (if none, ask) as the track description.
        2. Ask the user to enable Plan mode in Cursor (or SwitchMode → plan).
...
2.  **Questioning Phase:** Ask a series of questions to gather details for the `spec.md` using the `AskQuestion` tool. You must batch up to 4 related questions in a single tool call to streamline the process. Tailor questions based on the track type (Feature or Other).
...
1.  **Analyze Needs:**
    ...
    -   Identify any relevant skills that are NOT yet installed (check `~/.cursor/skills/` and `.cursor/skills/`).
2.  **Recommendation Loop:**
    -   **If relevant missing skills are found:**
        -   **Ask:** "Would you like to install these skills now?" using the `AskQuestion` tool (do not repeat in chat):
            - **questions:**
...
5.  **Write Files:**
    ...
    *   Write the index file to `<Tracks Directory>/<track_id>/index.md` with content:
        ...
        ```
6.  **Exit Plan Mode:** Ask the user to return to Agent mode when done.
