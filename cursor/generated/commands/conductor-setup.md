...
# Conductor — Scaffolds the project and sets up the Conductor environment
...
**CURSOR PLAN MODE:** For steps that require Plan Mode, ask the user to enable **Plan mode** in Cursor (or use SwitchMode → plan). You may create and edit only under `conductor/` using relative paths. Use `Write` and `StrReplace` — not shell redirection. Allowed shell: `git status`, `git diff`, `ls`, `mkdir`, `cp`, `git init`, `git add`, `git commit`.
...
1.  **Enter Plan Mode:** Ask the user to enable Plan mode in Cursor (or SwitchMode → plan).
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            -   **1.0 Pre-analysis Confirmation:**
                ...
                2.  **Ask for Permission:** Request permission for a read-only scan to analyze the project using the `AskQuestion` tool:
                    - **header:** "Permission"
            ...
            -   **2.1 File Size and Relevance Triage:**
                1.  **Respect Ignore Files:** Before scanning any files, you MUST check for the existence of `.cursorignore` and `.gitignore` files. If either or both exist, you MUST use their combined patterns to exclude files and directories from your analysis. The patterns in `.cursorignore` should take precedence over `.gitignore` if there are conflicts. This is the primary mechanism for avoiding token-heavy, irrelevant files like `node_modules`.
                ...
                3.  **Fallback to Manual Ignores:** ONLY if neither `.cursorignore` nor `.gitignore` exist, you should fall back to manually ignoring common directories. Example command: `ls -lR -I 'node_modules' -I '.m2' -I 'build' -I 'dist' -I 'bin' -I 'target' -I '.git' -I '.idea' -I '.vscode'`.
...
5.  **Inquire about Project Goal (for Greenfield):**
    -   **Ask the user the following question using the `AskQuestion` tool and wait for their response before proceeding to the next step:**
        - **header:** "Project Goal"
        - **type:** "text"
...
4.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single `AskQuestion` tool call to gather detailed requirements (e.g., target users, goals, features).
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single `AskQuestion` tool call to gather detailed preferences.
        -   **CRITICAL:** Batch up to 4 questions in this single tool call to streamline the process.
...
2.  **Determine Mode:**
    -   **FOR GREENFIELD PROJECTS:** Use the `AskQuestion` tool to choose the workflow.
        - **questions:**
            - **header:** "Tech Stack"
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Request Confirmation:** After stating the detected stack, you MUST ask the user for confirmation using the `AskQuestion` tool:
            - **questions:**
                - **header:** "Tech Stack"
                - **question:** "Is the inferred tech stack (listed above) correct?"
                - **type:** "yesno"
        -   **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately call the `AskQuestion` tool with `type: "text"` to allow the user to provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
2.  **Select Code Style Guides:**
    -   List the available style guides by using the `Shell` tool to execute `ls ~/.cursor/conductor/templates/code_styleguides/`. **CRITICAL: You MUST use `Shell` for this step. Do NOT use the `list_directory` tool, as the templates directory resides outside of your allowed workspace and the call will fail.**
    -   **FOR GREENFIELD PROJECTS:**
        ...
        -   **Determine Mode:** Use the `AskQuestion` tool:
            - **questions:**
        ...
        -   **If user chose "Select from Library":**
            ...
            -   **Action:** Announce "I'll present the available guides in groups. Please select all that apply." Then, immediately call the `AskQuestion` tool with the batched questions (do not list the questions in the chat).
            -   **Single Tool Call:** Create one `AskQuestion` call containing a `questions` array with one question per group.
...
3.  **Action:** Construct and execute a command to create the directory and copy all selected files. For example: `mkdir -p conductor/code_styleguides && cp ~/.cursor/conductor/templates/code_styleguides/python.md ~/.cursor/conductor/templates/code_styleguides/javascript.md conductor/code_styleguides/`
4.  **Continue:** Immediately proceed to the next section.

### 2.5 Select Workflow (Interactive)
1.  **Copy Initial Workflow:**
    -   Copy `~/.cursor/conductor/templates/workflow.md` to `conductor/workflow.md`.
2.  **Determine Mode:** Use the `AskQuestion` tool to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Workflow"
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Customize":**
        a. **Initial Batch:** Use a single `AskQuestion` tool call to gather primary customizations:
            - **questions:**
                - **header:** "Coverage"
        ...
        b. **Final Tweak (Second Batch):** Once the first batch is answered, immediately use a second `AskQuestion` tool call to show the result and allow for any additional tweaks:
            - **questions:**
                - **header:** "Workflow"
                - **type:** "text"
                - **question:**
...
1.  **Analyze and Recommend:**
    -   Read `skills/catalog.md` from the directory where the Conductor extension is installed (typically `~/.cursor/conductor/skills/catalog.md`).
...
2.  **Determine Mode:**
    ...
    - **If recommended skills are found:** Use the `AskQuestion` tool to present recommendations and choose an installation path.
    - **questions:**
        - **header:** "Agent Skills"
        - **question:**
...
3.  **Gather Selection (Conditional):**
    - **If user chose "Hand-pick":**
        ...
        - **Prompt for Selection:** Use the `AskQuestion` tool with a single question:
...
4.  **Process Selection:**
    ...
    -   If "Hand-pick": Parse the results from the `AskQuestion` call and install selected skills.
    -   If "Skip": Proceed without installation.
5.  **Installation Action:**
    -   For each selected skill:
        -   **Determine Installation Path:**
            - If `alwaysRecommend` is true, set the path to `~/.cursor/skills/<skill-name>/`.
            - Otherwise, set the path to `.cursor/skills/<skill-name>/`.
...
2.  **Notify and Pause:** **CRITICAL:** You MUST explicitly instruct the user: "New skills installed. New skills are in `~/.cursor/skills/` or `.cursor/skills/` — Cursor loads them automatically to enable them. Let me know when you have done this." Do NOT use the `AskQuestion` tool here.
...
5.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single `AskQuestion` tool call to gather detailed requirements.
...
7.  **User Confirmation Loop:**
    ...
    -   **Ask for Approval:** Use the `AskQuestion` tool to request confirmation. You MUST embed the drafted requirements directly into the `question` field so the user can review them.
        - **questions:**
            - **header:** "Review"
            - **question:**
...
3.  **Confirm Proposal:** Use the `AskQuestion` tool to validate the proposal:
    - **questions:**
...
4.  **Action:**
    ...
    -   **If user chose "Suggest changes":**
        -   Immediately call the `AskQuestion` tool again:
            - **header:** "New Track"
            - **type:** "text"
...
3.  **Generate Track Artifacts:**
    ...
    d. **Exit Plan Mode:** Ask the user to return to Agent mode when done.
