...
# Conductor — Reviews the completed track work against guidelines and the plan
...
1.  **Check for User Input:**
    -   The user provided the following arguments: text the user provided after the slash command (if none, ask).
...
1.  **Load Project Context:**
    ...
    -   **Check for Installed Skills:**
        -   Check for the existence of `.cursor/skills/` (Workspace tier) and `~/.cursor/skills/` (Extension tier).
...
3.  **Load and Analyze Changes (Smart Chunking):**
    ...
    -   **Strategy Selection:**
        ...
        -   **Large Changes (> 300 lines):**
            -   **Confirm:** Immediately call the `AskQuestion` tool to confirm before proceeding with a large review (do not repeat the question in the chat):
                - **questions:**
                    - **header:** "Large Review"
...
2.  **Action:**
    -   **If issues found:** Immediately call the `AskQuestion` tool (do not repeat the question in the chat):
        - **questions:**
            - **header:** "Decision"
...
2.  **Condition for Action:**
    ...
    -   If changes are detected:
        a. **Check for Track Context:**
            - If you are NOT reviewing a specific track (i.e., you don't have a `plan.md` in context), immediately call the `AskQuestion` tool (do not repeat the question in the chat):
                - **questions:**
